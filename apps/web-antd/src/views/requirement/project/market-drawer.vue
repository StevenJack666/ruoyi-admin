...
import { addProject, editProject, projectInfo } from '#/api/requirement/project';
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
  ...
  async onOpenChange(isOpen) {
    ...
    if (isUpdate.value && data?.id) {
      try {
        const record = await projectInfo(data.id);
...
async function handleConfirm() {
  try {
    ...
    //   }
    // }
    await (isUpdate.value ? editProject(data) : addProject(data));
    resetInitialized();
    emit('reload');
    drawerApi.close();
  } catch (error) {
